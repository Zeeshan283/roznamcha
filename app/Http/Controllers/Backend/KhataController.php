...
class KhataController extends Controller
{
    ...
    public function index()
    {
        ...
        $ghulamkhan = GhulamkhanOrders::select('id','malwala', 'musalsal_num', 'comission');
        $kharlachi = KharlachiOrders::select('id','malwala', 'musalsal_num', 'comission');
        $thorkham = ThorkhamOrders::select('id','malwala', 'musalsal_num', 'comission');
        $wana = WanaOrders::select('id','malwala', 'musalsal_num', 'comission');

        $roznamchas_afg =$ghulamkhan->union($kharlachi)->union($thorkham)->union($wana)->get();


        // $records = $ghulamkhan->union($kharlachi)->union($thorkham)->union($wana)->get();
