    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-7 align-self-center">
                <h4 class="page-title text-truncate text-dark font-weight-medium mb-1">Thorkham Khan</h4>
                <div class="d-flex align-items-center">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb m-0 p-0">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"
                                    class="text-muted">Dashboard</a></li>
                            <li class="breadcrumb-item text-muted active" aria-current="page">Order Thorkham Khan</li>
                        </ol>
                    </nav>
                </div>
            </div>
            <div class="col-5 align-self-center">
                <div class="customize-input float-end">
                    @if (Auth::guard('admin')->user()->can('orders.thorkham.edit'))
                        <a class="btn btn-rounded btn-primary" href="{{ route('admin.orders.thorkham.create') }}">Create New
                            Order</a>
                    @endif
                </div>
            </div>
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Thorkham Khan Orders</h4>
                        ...
                        <div class="table-responsive">
                            <table id="multi_col_order" class="table border table-striped table-bordered text-nowrap"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($ThorkhamOrders as $data)
                                        <tr>
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-content" id="dropdownContent">
                                                        ...
                                                        <a href="{{ route('admin.orders.thorkham.destroy1', $data->id) }}"
                                                            class="fa fa-trash text-black"
                                                            data-record-id="{{ $data->id }}">Delete</a>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>{{ $data['date'] }}</td>
                                            ...
                                            <td>
                                                <a
                                                    href="{{ route('admin.khata.personal.index', $data['admin']['id']) }}">{{ $data['admin']['name'] }}</a>
                                            </td>
                                            ...
                                            @if ($matchedSelf)
                                                <td><a
                                                        href="{{ route('admin.khata.personal.index', $matchedexpense['admin']['id']) }}">{{ $matchedexpense['admin']['name'] }}</a>
                                                </td>
                                                <td>{{ $matchedexpense['comission'] }}</td>
                                            @else
                                                <td colspan="2">No Self Records for Musalsal Num: <span
                                                        style="color: red">{{ $data['musalsal_num'] }}</span></td>
                                            @endif

                                            @if ($matchedSelf)
                                                <td>{{ $matchedSelf['us_malwala'] }}</td>
                                                <td>{{ $matchedSelf['exchange_rate'] }}</td>
                                                <td>{{ $matchedSelf['us_malwala'] * $matchedSelf['exchange_rate'] ?? null }}
                                                </td>
                                            @else
                                                <td colspan="2">No Self Records for Musalsal Num: <span
                                                        style="color: red">{{ $data['musalsal_num'] }}</span></td>
